...
import { CsvAdapter, FileValidator, SchemaValidationError, ParseError } from '../src/s2-storage';
// ===== TEST DATA =====
const validEntries = [
...
function runTests() {
    ...
    test('CSV: Parse valid CSV', () => {
        ...
        if (result[0].hours !== 8)
            throw new Error('Hours mismatch');
    });
    test('CSV: Parse quoted fields', () => {
        const csv = 'date,hours,person_id,task_id,note\n2026-01-01,8,p1,t1,"Note, with comma"';
        const result = CsvAdapter.csvToWorkLog(csv);
        if (result[0].note !== 'Note, with comma')
            throw new Error('Quoted field failed');
    });
    test('CSV: Generate CSV', () => {
        const csv = CsvAdapter.workLogToCsv(validEntries);
        if (!csv.includes('2026-01-01,8,p1,t1,"Test"'))
    ...
    test('Validator: Reject invalid person_id', () => {
        const invalid = [Object.assign(Object.assign({}, validEntries[0]), { person_id: 'invalid' })];
        try {
