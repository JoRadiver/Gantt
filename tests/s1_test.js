...
test("rejects empty id", () => {
    const r = validatePerson(Object.assign(Object.assign({}, defaultPerson({ id: "x", name: "A" })), { id: "" }));
    expect(r.ok).toBeFalsy();
    if (!r.ok)
        expect(r.errors[0].field).toBe("id");
});
test("rejects initials > 2 chars", () => {
    const r = validatePerson(Object.assign(Object.assign({}, defaultPerson({ id: "p1", name: "A" })), { initials: "ABC" }));
    expect(r.ok).toBeFalsy();
});
test("rejects unknown palette key", () => {
    const r = validatePerson(Object.assign(Object.assign({}, defaultPerson({ id: "p1", name: "A" })), { color: "pink" }));
    expect(r.ok).toBeFalsy();
});
// ── validateTask ──────────────────────────────────────────────────────────────
console.log("\nvalidateTask");
...
test("rejects milestone with estimated_hours > 0", () => {
    ...
    const bad = Object.assign(Object.assign({}, t), { estimated_hours: 5 });
    const r = validateTask(bad);
    expect(r.ok).toBeFalsy();
});
test("rejects milestone with end_date != start_date", () => {
    const t = defaultTask({ id: "m1", title: "M", type: "milestone" });
    const bad = Object.assign(Object.assign({}, t), { end_date: "2099-01-01" });
    const r = validateTask(bad);
    expect(r.ok).toBeFalsy();
});
...
test("rejects invalid completion_derived", () => {
    const t = Object.assign(Object.assign({}, defaultTask({ id: "t1", title: "X" })), { completion_derived: 1.5 });
    expect(validateTask(t).ok).toBeFalsy();
});
test("rejects non-integer end_date_flexibility_range", () => {
    const t = Object.assign(Object.assign({}, defaultTask({ id: "t1", title: "X" })), { end_date_flexibility_range: 2.5 });
    expect(validateTask(t).ok).toBeFalsy();
});
// ── validateDependency ────────────────────────────────────────────────────────
console.log("\nvalidateDependency");
...
test("rejects non-integer lag_days", () => {
    const d = Object.assign(Object.assign({}, defaultDependency({ id: "d1", from_task_id: "t1", to_task_id: "t2" })), { lag_days: 1.5 });
    expect(validateDependency(d).ok).toBeFalsy();
});
test("accepts negative lag_days (lead time)", () => {
    const d = Object.assign(Object.assign({}, defaultDependency({ id: "d1", from_task_id: "t1", to_task_id: "t2" })), { lag_days: -2 });
    expect(validateDependency(d).ok).toBeTruthy();
});
// ── validateWorkEntry ─────────────────────────────────────────────────────────
console.log("\nvalidateWorkEntry");
...
test("rejects hours = 0", () => {
    const e = Object.assign(Object.assign({}, defaultWorkEntry({ id: "w1", person_id: "p1", task_id: "t1", hours: 4 })), { hours: 0 });
    expect(validateWorkEntry(e).ok).toBeFalsy();
});
test("rejects invalid date format", () => {
    const e = Object.assign(Object.assign({}, defaultWorkEntry({ id: "w1", person_id: "p1", task_id: "t1", hours: 1 })), { date: "01-06-2025" });
    expect(validateWorkEntry(e).ok).toBeFalsy();
});
...
test("rejects unknown parent_id", () => {
    const bad = Object.assign(Object.assign({}, validProject), { tasks: [Object.assign(Object.assign({}, defaultTask({ id: "t1", title: "X" })), { parent_id: "ghost" })], dependencies: [] });
    expect(validateProject(bad).ok).toBeFalsy();
});
test("rejects parent that is not a group", () => {
    const bad = Object.assign(Object.assign({}, validProject), { tasks: [
            defaultTask({ id: "t1", title: "Parent task", type: "task" }),
            Object.assign(Object.assign({}, defaultTask({ id: "t2", title: "Child" })), { parent_id: "t1" }),
        ], dependencies: [] });
    expect(validateProject(bad).ok).toBeFalsy();
});
test("rejects dependency referencing unknown task", () => {
    const bad = Object.assign(Object.assign({}, validProject), { dependencies: [defaultDependency({ id: "d1", from_task_id: "t1", to_task_id: "ghost" })] });
    expect(validateProject(bad).ok).toBeFalsy();
});
test("rejects task with unknown person_id", () => {
    const bad = Object.assign(Object.assign({}, validProject), { tasks: [Object.assign(Object.assign({}, defaultTask({ id: "t1", title: "X" })), { people_ids: ["ghost_person"] })], dependencies: [] });
    expect(validateProject(bad).ok).toBeFalsy();
});
...
test("all 8 palette keys are present", () => {
    ...
    for (const k of keys) {
        if (!(k in PALETTE))
            throw new Error(`Missing palette key: ${k}`);
    }
});
